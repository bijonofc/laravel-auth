...
`Appsbd\Auth\Services\GoogleOAuthService` implements `Appsbd\Auth\Contracts\OAuthProviderInterface`. Resolve it via DI, `app(OAuthProviderInterface::class)`, or the `GoogleOAuth` facade.
...
DTOs (`Appsbd\Auth\Support`, readonly):
...
|---|---|---|
| `Appsbd\Auth\Events\GoogleLoginSucceeded` | `OAuthUser $user`, `OAuthTokens $tokens` | `callback()` completes successfully |
| `Appsbd\Auth\Events\GoogleLoginFailed` | `string $reason`, `?Throwable $exception` | any `callback()` failure (state, code, HTTP) |
...
Raw HTTP client exceptions never leak; everything is mapped to `OAuthException` (all package exceptions extend `Appsbd\Auth\Exceptions\AuthException`).
